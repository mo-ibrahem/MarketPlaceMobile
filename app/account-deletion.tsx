...
export default function AccountDeletionScreen() {
 ...
 useEffect(() => {
   ...
   async function refresh() {
     ...
     if (mounted) setStatus(error || !data?.[0] ? 'unavailable' : data[0].status);
   }
   void refresh();
 ...
 return <SafeAreaView style={styles.page}><View style={styles.card}>
   ...
   <Text style={styles.text}>{status === 'complete'
   ...
   {status === 'unavailable' && <Text style={styles.text}>Could not retrieve the status. Please try again later.</Text>}
